...
/// for Flutter, which the core does not need either.
...
void main() {
  ...
  test('the scan finds a planted import', () {
    final directory = Directory.systemTemp.createTempSync('peek_talker_arch');
    addTearDown(() => directory.deleteSync(recursive: true));

    final nested = Directory('${directory.path}/nested')..createSync();
    File('${nested.path}/offender.dart')
      ..createSync()
      ..writeAsStringSync("import 'package:peek/peek.dart';");
  ...
  test('the detector reads directives, not any mention of the library', () {
    ...
    expect(reachesPastCore("import 'package:talker/talker.dart';"), isFalse);
